...
Spending a few seconds to properly name your folders & files will help you find your files more quickly later & it’ll look more professional.
Following are some points you should always keep in mind when creating & organizing folders & files.
...
**Point 3** 	Being consistent in naming your folder by following a naming format will make it easier for you identify the correct folder especially if you have different projects of same company at a time. Choose a naming format & stick with it.
...
**Point 6** Be descriptive in name but keep it short & logical.  Don't go overboard - file/path names have length limits which vary between operating systems. Under Windows the maximum full path length for a file (e.g. the drive letter + folder names + file name) is 260 characters.
Use common abbreviations wherever possible, such as Jan for January or Corp for Corporation. The goal when naming files is to be able to tell what the file is about without having to open it and look.
...
# Step 1: Type command git branch branch_name.
# Step 2: To switch among branches git checkout branch_name.
# Step 3: To check in which branch currently working in/list of all branches git branches. 
