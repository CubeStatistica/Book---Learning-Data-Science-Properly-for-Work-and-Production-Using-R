...
```
Reference link: https://realpython.com/list-comprehension-python/
...
Implement bag of words in python. Given a body of text list every word and count the number of times it occurs.

So for each word would have a corresponding count. A dictionary is a natural datatype for such a problem due to its key-value pair structure.

```{python}
peom = "My poem after a long time. I shall write a poem praising the Creature and working hard."
print(peom.split())
a = dict()
a["a"] = 1
print(a)
...
poem = "My poem after a long time. I shall write a poem praising the Creature and working hard."

# fun || input: string ==> output: dict
...
def bag_of_words(text):
    ...
    for word in text.split():
        #print(word)
        bagWordsDict[word] = bagWordsDict.get(word, 0) + 1
    return bagWordsDict

print(bag_of_words(poem))

```

__One of the important key differences between python and R is the return statement in a function. In R the return command is a command and it works like a function and it takes input the object that needs to be returned. So in R you would end the function to return object `obj_returned` would be written as follows:  `return(obj_returned)`. This is the same how something is printed in R `print(obj_returned)`. In python 3.* printing works the same however, the return statement is not used as a function as is in R and is just a statement and would be written as follows: `return obj_returned`. There are no `()`. And if you do write `return(obj_returned)` in python it can cause issues. For example if the returned object is a dictionary you will receive the following error: `TypeError: 'dict' object is not callable`.__


For improved presentation we will convert the dictionary to a pandas data frame.

First we load the `pandas` package. It is assumed it has already been downloaded in the machine.
...
Bag_Of_Words = pd.DataFrame('words' = words_dict.keys(), 'count' = words_dict.values()
)
